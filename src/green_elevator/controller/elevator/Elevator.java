...
public class Elevator implements Runnable {
	...
	while (true) {
	    ...
	    if (outOfOrder.get()) {
		if (debug)
		    System.out.println("Move to goal floor recognize out of order");
		sendStopMessage();
		return;
	    }
	...
	if (message.getMessageType() == MessageType.STOPMESSAGE) {
	    System.out.println("Out of order was set to true for elavator " + id);
	    outOfOrder.set(true);
	    return true;
	}
