...
public class Evaluator implements Runnable {
	...
	while (true) {
	    ...
	    Optional<Integer> bestElevatorId = findOptimalElevator(message);
	    if (!bestElevatorId.isPresent()) {
		break;
	    }
	    elevators.get(bestElevatorId.get()).addTask(message);
	}
	System.exit(1);

    }

    private Optional<Integer> findOptimalElevator(OutsideMessage message) {
	...
	// Try to first take an elevator from the group which is in the right
	// direction with the shortest path to the
	// wanted floor
	if (!elevatorsInRightDirection.isEmpty())
	    return Optional.of(elevatorWithShortestPath(elevatorsInRightDirection, currentPositions, wantedFloor));

	// Else we try to get a static elevator with the shortest path to the
	// wanted floor
	if (!staticElevators.isEmpty()) {
	    return Optional.of(elevatorWithShortestPath(staticElevators, currentPositions, wantedFloor));
	}
	// If we have no other option we take the elevator with the current
	// least tasks
	return elevatorWithLeastTasks(elevatorsInWrongDirection);
    }

    private Optional<Integer> elevatorWithLeastTasks(Set<Integer> elevatorsInWrongDirection) {
	int minTaskAmount = Integer.MAX_VALUE;
	Optional<Integer> bestElevatorId = Optional.empty();
	for (int id : elevatorsInWrongDirection) {
	    int nTasks = elevators.get(id).numberOfTasks();
	    if (nTasks < minTaskAmount) {
		minTaskAmount = nTasks;
		bestElevatorId = Optional.of(id);
	    }
	}
